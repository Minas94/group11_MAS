...
/**
 * This is the decoupled Offering Strategy for Agent K (ANAC2010). The code was
 * taken from the ANAC2010 AgentK and adapted to work within the BOA framework.
 *
 * OPPONENT MODEL EXTENSION The strategy was extended to incorperate an opponent
 ...
 */
public class AgentK_Offering extends OfferingStrategy {
        ...
        public BidDetails determineNextBid() {
                if (negotiationSession.getOpponentBidHistory().getHistory().size() > 0) {
                        ((AgentKSAS) helper).calculateAcceptProbability();
                }

                ArrayList<BidDetails> bidTemp = new ArrayList<BidDetails>();

                for (Bid bid : ((AgentKSAS) helper).getOfferedBidMap().keySet()) {
                        double bidUtil = ((AgentKSAS) helper).getOfferedBidMap().get(bid);
                        if (bidUtil > ((AgentKSAS) helper).getTarget()) {
                ...
                if (size > 0) {
                        if (opponentModel instanceof NoModel) {
	                        ...
	                        if (opponentModel instanceof NoModel) {
	                                try {
	                                        int loop = 0;
	                                        while (searchUtil < ((AgentKSAS) helper).getBidTarget()) {
	                                                if (loop > 500) {
	
	                                                        ((AgentKSAS) helper).decrementBidTarget(0.01);
	                                                        loop = 0;
	                                                }
	                                                nextBid = searchBid();
	                                ...
	                                nextBid = omStrategy.getBid(outcomespace, ((AgentKSAS) helper).getBidTarget());
	                        }
	                }
	                return nextBid;
	        }
