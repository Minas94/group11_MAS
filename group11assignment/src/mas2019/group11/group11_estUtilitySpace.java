...
public class group11_estUtilitySpace extends AbstractNegotiationParty {
	/** This agent will make bids above the minimuTarget */
	private double minimumTarget = 1;
	//ArrayList<OutcomeComparison> bidCompare = new ArrayList<OutcomeComparison>();
	...
	public void init(NegotiationInfo info) {
		...
		log("Lowest util: " + userModel.getBidRanking().getLowUtility() + ". Highest util: "
				+ userModel.getBidRanking().getHighUtility());
		...
		if (userModel instanceof ExperimentalUserModel) {
			...
			log("The real utility of the random bid is: " + realUSpace.getUtility(randomBid));
		}
	}
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		if (getLastReceivedAction() instanceof Offer) {
			...
			// Accept in the last 3 rounds if the received bid is better than the
			// reservation value
	...
	/**
	 * We override the default estimate of the utility space by using
	 * {@link ClosestKnownBid} defined below.
	 */
	@Override
	...
	/**
	 * Defines a custom UtilitySpace based on the closest known bid to deal with
	...
	private class ClosestKnownBid extends CustomUtilitySpace {
		...
		public double estimateUtilityOfRankedBid(Bid b) {
			...
			ArrayList<OutcomeComparison> bidCompare =  (ArrayList<OutcomeComparison>) bidRanking.getPairwiseComparisons();
			...
			for (int i = 0; i<bidCompare.size(); i++) {
				OutcomeComparison TwoBids = bidCompare.get(i);
				Bid Bid1 = TwoBids.getBid1();
				Bid Bid2 = TwoBids.getBid2();
				List<Issue> IssueListBid1 = Bid1.getIssues();
				List<Issue> IssueListBid2 = Bid2.getIssues();
				
				
				
				for(int j = 0; j<IssueListBid1.size();j++) {
					Issue issueCompare1 =IssueListBid1.get(j);
					Issue issueCompare2 = IssueListBid2.get(j);
				}
			}
			...
			return min + i * (max - min) / ((double) bidRanking.getSize()/2);
		}

		/**
		 * Finds the bid in the bid ranking that is most similar to bid given in the
		 * argument bid
		 */
	...
	/*
	 * @Override public AbstractUtilitySpace estimateUtilitySpace() { return
	 * newUtilitySpaceEstimator(getDomain(), userModel); }
	 * 
	 * 
	 * 
	 * public static AbstractUtilitySpace newUtilitySpaceEstimator(Domain domain,
	 * UserModel um) { AdditiveUtilitySpaceFactory factory = new
	 * AdditiveUtilitySpaceFactory(domain); BidRanking bidRanking =
	 * um.getBidRanking(); factory.estimateUsingBidRanks(bidRanking); return
	 * factory.getUtilitySpace();
	 * 
	 * }
	 * 
	 * public void estimateUsingBidRanks(BidRanking r) { double points = 0; for (Bid
	 * b : r.getBidOrder()) { List<Issue> issues = b.getIssues(); for (Issue i :
	 * issues) { int no = i.getNumber(); ValueDiscrete v = (ValueDiscrete)
	 * b.getValue(no); double oldUtil = getUtility(i, v); setUtility(i, v, oldUtil +
	 * points); } points += 1; } normalizeWeightsByMaxValues(); }
	 * 
	 * @Override public Action chooseAction(List<Class<? extends Action>>
	 * possibleActions) { // TODO Auto-generated method stub return null; }
	 * 
